...
export const useBinStore = () => {
  ...
  const { binProducts, activeBinProduct, binUsers, activeBinUser } =
    useSelector((state) => state.bin);

  /* PRODUCTS */

  const startLoadBinProducts = async (
    filterBy,
    orderBy,
    searchText,
    page = 1
    ...
    try {
      ...
      const {
        ...
        `bin/products/${term}?page=${page}&filterBy=${filterBy}&orderBy=${orderBy}`
      );
  ...
  const productBinStartEnable = async () => {
    try {
      ...
      const {
        data: { record, msg },
      } = await dashboardApi.put(`bin/products/${activeBinProduct._id}`, {});
      ...
      if (msg === "OK") {
        ...
        dispatch(
          uiOpenSuccessAlert("El producto fue habilitado exitosamente!")
        );
      } else {
        dispatch(uiCloseProgressBackdrop());
        dispatch(
          uiOpenErrorAlert(
            "Error al intentar habilitar el producto! Hable con el administrador"
          )
        );
        console.log(msg);
      }
  ...
  const productBinStartDeleted = async (product) => {
    try {
      ...
      const {
        data: { record, msg },
      } = await dashboardApi.delete(`bin/products/${product._id}`, {});
      ...
      if (msg === "OK") {
        ...
        dispatch(
          uiOpenSuccessAlert(
            "El producto fue removido de forma permanente exitosamente!"
          )
        );
      } else {
        dispatch(uiCloseProgressBackdrop());
  ...
  const startLoadBinUsers = async (filterBy, orderBy, searchText, page = 1) => {
    ...
    try {
      ...
      const {
        ...
        `bin/users/${term}?page=${page}&filterBy=${filterBy}&orderBy=${orderBy}`
      );
  ...
  const userBinStartEnable = async () => {
    try {
      ...
      const {
        data: { msg, record },
      } = await dashboardApi.put(`bin/users/${activeBinUser._id}`, {});
      ...
      if (msg === "OK") {
        ...
        dispatch(
          uiOpenErrorAlert(
            "Error al intentar habilitar el usuario! Hable con el administrador"
          )
        );
        console.log(msg);
      }
  ...
  const userBinStartDeleted = async (user) => {
    try {
      ...
      const {
        data: { msg, record },
      } = await dashboardApi.delete(`bin/users/${user._id}`, {});
      ...
      if (msg === "OK") {
        ...
        dispatch(
          uiOpenSuccessAlert(
            "El usuario fue removido de forma permanente exitosamente!"
          )
        );
      } else {
        dispatch(uiCloseProgressBackdrop());
        dispatch(
          uiOpenErrorAlert(
            "Error al intentar eliminar el usuario! Hable con el administrador"
          )
        );
        console.log(msg);
      }
